...
<%-- 	                                         <td>${notice.noticeSeq }</td> --%>
	                                         <td>${i + 1 + noticeFrontNum}</td>
	                                         <td style="text-align: left;"><a href="/noticeDetail?noticeSeq=${NoticeList.get(i).noticeSeq }">${NoticeList.get(i).title }</a></td>
	                                         <td>${NoticeList.get(i).empId } <input type="hidden" name="empId" value="${NoticeList.get(i).empId }"></td>
	                                         <td>
											 <fmt:formatDate value="${NoticeList.get(i).updDt }" pattern="yyyy-MM-dd"/>
											 </td>
	                                     </tr>
                                   	</c:forEach>
                                </tbody>

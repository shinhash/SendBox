...
<script>
	...
	$(document).ready(function() {
		...
		} else if(${param.searchCondition == 'receiver'}){
			$('#searchType').val('receiver')
		}
				...
				if(result){
					$.ajax({
						...
						success : function(data) {
							window.location.href = "/mail/sendList";
						}
					})
				}
			}
		...
		$('#searchBtn').on('click',function(){
			...
			if(searchKeyword == ''){
				alert('키워드를 입력해주세요')
				location.href = "/mail/sendList";
			} else {
				location.href = "/mail/sendList?searchCondition=" +searchCondition+ "&searchKeyword=" + searchKeyword;
			}
		})
            ...
            <div class="container-fluid">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text"><i class="fas fa-paper-plane" style="margin-left: 10px;"></i>&nbsp;보낸 메일함</h6>
                    </div>
                    <div class="card-body">
												...
												<c:otherwise>
													<li class="paginate_button page-item previous" id="dataTable_previous">
								                        ...
								                        <c:when test="${param.searchCondition == null}">
															<a href="/mail/sendList" aria-controls="dataTable"  class="page-link">
																<i class="fas fa-angle-double-left"></i>
															</a>
								                        </c:when>
								                        <c:when test="${param.searchCondition != null}">
															<a href="/mail/sendList?searchCondition=${param.searchCondition }&searchKeyword=${param.searchKeyword }" aria-controls="dataTable"  class="page-link">
																<i class="fas fa-angle-double-left"></i>
															</a>
								                        </c:when>
							                        	</c:choose>
													...
													<li class="paginate_button page-item previous" id="dataTable_previous">
														<c:choose>
								                        <c:when test="${param.searchCondition == null}">
															<a href="/mail/sendList?pageIndex=${pagination.currentPageNo-1}" aria-controls="dataTable"  class="page-link">
																<i class="fas fa-angle-left"></i>
															</a>
								                        </c:when>
								                        <c:when test="${param.searchCondition != null}">
															<a href="/mail/sendList?pageIndex=${pagination.currentPageNo-1}&searchCondition=${param.searchCondition }&searchKeyword=${param.searchKeyword }" aria-controls="dataTable"  class="page-link">
																<i class="fas fa-angle-left"></i>
															</a>
								                        </c:when>
							                        	</c:choose>
													...
													<c:choose>
								                        <c:when test="${param.searchCondition == null}">
															<li class="paginate_button page-item">
																<a href="/mail/sendList?pageIndex=${i}" aria-controls="dataTable"  class="page-link">${i }</a>
															</li>
								                        </c:when>
								                        <c:when test="${param.searchCondition != null}">
															<li class="paginate_button page-item">
																<a href="/mail/sendList?pageIndex=${i}&searchCondition=${param.searchCondition }&searchKeyword=${param.searchKeyword }" aria-controls="dataTable"  class="page-link">${i }</a>
															</li>
								                        </c:when>
							                        	</c:choose>
													</c:otherwise>
											...
											<c:choose>
												...
												<c:otherwise>
													<li class="paginate_button page-item next" id="dataTable_next">
														<c:choose>
								                        <c:when test="${param.searchCondition == null}">
															<a href="/mail/sendList?pageIndex=${pagination.currentPageNo+1}" aria-controls="dataTable" data-dt-idx="2" tabindex="0" class="page-link">
																<i class="fas fa-angle-right"></i>
															</a>
								                        </c:when>
								                        <c:when test="${param.searchCondition != null}">
															<a href="/mail/sendList?pageIndex=${pagination.currentPageNo+1}&searchCondition=${param.searchCondition }&searchKeyword=${param.searchKeyword }" aria-controls="dataTable" data-dt-idx="2" tabindex="0" class="page-link">
																<i class="fas fa-angle-right"></i>
															</a>
								                        </c:when>	
							                        	</c:choose>
													...
													<li class="paginate_button page-item next" id="dataTable_next">
														<c:choose>
								                        <c:when test="${param.searchCondition == null}">
															<a href="/mail/sendList?pageIndex=${pagination.getTotalPageCount()}" aria-controls="dataTable" data-dt-idx="2" tabindex="0" class="page-link">
																<i class="fas fa-angle-double-right"></i>
															</a>
								                        </c:when>
								                        <c:when test="${param.searchCondition != null}">
															<a href="/mail/sendList?pageIndex=${pagination.getTotalPageCount()}&searchCondition=${param.searchCondition }&searchKeyword=${param.searchKeyword }" aria-controls="dataTable" data-dt-idx="2" tabindex="0" class="page-link">
																<i class="fas fa-angle-double-right"></i>
															</a>
								                        </c:when>
							                        	</c:choose>

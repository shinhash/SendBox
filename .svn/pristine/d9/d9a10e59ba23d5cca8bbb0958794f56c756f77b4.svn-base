...
//	}
	...
	public String deleteMeeting(String id,MeetingVO vo, HttpSession session,Model model) {
		...
		try {
			...
			logger.debug("333333333333333 : {}", empService.getdeptId(empVo).getDeptName());
			model.addAttribute("deptname",empService.getdeptId(empVo).getDeptName());
			
			logger.debug("vo22222222222222 : {}", vo2);
		} catch (Exception e) {
			e.printStackTrace();
		}

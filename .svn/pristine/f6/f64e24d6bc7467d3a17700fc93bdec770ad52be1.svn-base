...
<!-- 			<i class="fas fa-fw fa-user-lock" style="font-size: 20px; "></i> -->
		...
		<td id="td2">
		<div class="card shadow md-3" style="width: 100%; height: 315px; overflow: auto;">
			<div class="card-header py-3">
...
<canvas id="weekcanvas" width ="400" height = "250"></canvas>
			...
			<div class="card shadow md-3" style="width: 100%; overflow: auto;">
				<div class="card-header py-3 card-tab">
		<!-- 		<i class="far fa-file-alt" style="font-size: 20px; "></i> -->
					<div id="report" class="m-0 font-weight-bold text-default tab active-tab" style="width: 155px;" onclick="javascript:tabClick(this)">
						📑 진행중인 서류
					</div>
					<c:if test="${EMP.authLv >= 1}">
						<div id="approval" class="m-0 font-weight-bold text-default tab" style="width: 145px;" onclick="javascript:tabClick(this)" >
							📝 결재할 서류
						</div>
						<c:if test="${subWorker != null }">
							<div id="subApproval" class="m-0 font-weight-bold text-default tab" style="width: 158px;" onclick="javascript:tabClick(this)" >
								📝 대리 결재 서류
							</div>
						</c:if>					
					</c:if>
					...
					<script type="text/javascript">
						function tabClick(div){
							...
							if(div.id === 'approval' || div.id ==='subApproval'){
								aTag.setAttribute('href', '/report/approvListView');
								console.log(aTag.href);
							}
							
							getListAjax(aTag.href, div.id);
						}
						
						function getListAjax(url, id){
							var subEmpId = '';
							if(id === 'subApproval'){
								subEmpId = "${subWorker.subEmpId}";
							}
							$.ajax({
								url : url,
			...
			<td id="td3">
			<div class="card shadow md-3" style="width: 100%; height: 390px;">
					<div class="card-header py-3">
...
$(function(){
	...
	$.ajax({
		...
		success : function(data){
			...
			myUseVacateCnt = parseFloat(data.myUseVacateCnt);
			myAllVacateCnt = parseFloat(data.myVacateDayCnt - myUseVacateCnt);

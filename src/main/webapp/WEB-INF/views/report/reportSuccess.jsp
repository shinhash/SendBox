...
$(document).ready(function(){
	...
	$('#recordCnt').change(function(){
		...
		document.location = "/reportWait?recordCountPerPage="+recordCountPerPage;
	})
});	
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <!-- Page Heading -->
                    <h2 class="h3 mb-2 text-gray-800"><i class="far fa-file-alt" style="margin-left: 10px;"></i>&nbsp&nbsp서류완료함</h2>
                    <br>
                    <!-- DataTales Example -->
                                ...
                                <table class="table table-bordered" id=""  cellspacing="0" style="text-align: center;" >
                                    ...
                                    <tbody id="reportList"  style="text-align: center;">
                                      <c:forEach items="${reportList}" var="list" varStatus="status"> 
                                        <tr class="dataTr" data-a="${list.reportId }">
                                        	<td>${(pagination.currentPageNo-1) * pagination.pageSize + status.index+1}</td>
                                            <td>${list.reportId }</td>
                                            <td align="left">&nbsp;&nbsp;${list.title }</td>
                                            <td>
	                                            <c:choose>
	                                            	<c:when test="${list.typeId =='T1'}">기안서</c:when>
	                                            	<c:when test="${list.typeId =='T2'}">휴가</c:when>
	                                            	<c:when test="${list.typeId =='T3'}">월차</c:when>
	                                            	<c:when test="${list.typeId =='T4'}">반차</c:when>
	                                            	<c:when test="${list.typeId =='T5'}">사직서</c:when>
	                                            </c:choose>
                                            </td>
                                            <td>${list.deptName }</td>
                                            <td>${list.empNm }</td>
                                            <td>
                                            	<fmt:parseDate var="reportDt" value="${list.reportDt }" pattern="yyyy-MM-dd HH:mm"/>
                                   ...
                                   <c:if test="${fn:length(reportList) == 0 }">
                                         <tr>
                                            <td colspan="7" style="text-align: center;">서류가 없습니다</td>
                                         </tr>
                                      </c:if>
											...
											<c:choose>
												...
												<c:otherwise>
													<li class="paginate_button page-item previous" id="dataTable_previous">
														<a href="/reportSuccess" aria-controls="dataTable"  class="page-link">
															<i class="fas fa-angle-double-left"></i>
														</a>
													</li>
													<li class="paginate_button page-item previous" id="dataTable_previous">
														<a href="/reportSuccess?pageIndex=${pagination.currentPageNo-1}" aria-controls="dataTable"  class="page-link">
															<i class="fas fa-angle-left"></i>
														</a>
													</li>
												</c:otherwise>
												...
												<c:choose>
													...
													<c:otherwise>
														<li class="paginate_button page-item">
															<a href="/reportSuccess?pageIndex=${i}" aria-controls="dataTable"  class="page-link">${i }</a>
														</li>
													</c:otherwise>
												</c:choose>
											</c:forEach>
											<c:choose>
												...
												<c:otherwise>
													<li class="paginate_button page-item next" id="dataTable_next">
														<a href="/reportSuccess?pageIndex=${pagination.currentPageNo+1}" aria-controls="dataTable" data-dt-idx="2" tabindex="0" class="page-link">
															<i class="fas fa-angle-right"></i>
														</a>
													</li>
													<li class="paginate_button page-item next" id="dataTable_next">
														<a href="/reportSuccess?pageIndex=${pagination.getTotalPageCount()}" aria-controls="dataTable" data-dt-idx="2" tabindex="0" class="page-link">
															<i class="fas fa-angle-double-right"></i>
														</a>
													</li>
												</c:otherwise>
